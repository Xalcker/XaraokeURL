...
* **Vista minimalista (`?ui=minimal`), automática en TV:** sin barras laterales, con el video a pantalla completa y "ahora suena" / "a continuación" / logo / QR flotando como overlays translúcidos en cada esquina — la misma estética del reproductor nativo (ver más abajo), pero en el navegador. No muestra la lista de próximas 5 canciones. Se activa sola en un navegador de TV (Fire TV/Silk, Samsung Tizen, LG webOS, Android TV/Google TV, detectados por el user agent en `public/js/uiMode.js`), porque ahí nadie va a navegar una barra lateral con el control remoto del televisor. En cualquier otro navegador se agrega a mano a la URL, por ejemplo `http://<servidor>:8081/?autostart=1&ui=minimal`; y en una TV se puede volver a la clásica con `?ui=classic`.
