      ...
      run(SCHEMA)
        .then(() =>
          resolve({
            ...
            // Los pulgares arriba y abajo de todas las canciones calificadas, sumados de todas las
            // salas: { song_key: { up, down } }. Las canciones sin calificaciones no aparecen.
            async totals() {
              const rows = await all(
                `SELECT song_key, SUM(value = 1) AS up, SUM(value = -1) AS down
                 FROM ratings GROUP BY song_key`
              );
              return Object.fromEntries(rows.map((r) => [r.song_key, { up: r.up, down: r.down }]));
            },
            close() {
