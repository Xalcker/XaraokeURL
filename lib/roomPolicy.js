...
const DEFAULT_SONG_COUNTDOWN_SECONDS = 5;
const MAX_SONG_COUNTDOWN_SECONDS = 30;

// Interpreta SONG_COUNTDOWN_SECONDS: la cuenta regresiva, como la de las películas, que la pantalla
// principal muestra antes de cada canción para que quien canta tome el micrófono. Devuelve
// { seconds, warning }:
// - Si la variable no está definida, se usan 5 segundos.
// - Con 0 no hay cuenta regresiva: la canción empieza en cuanto se carga.
// - Se redondea a segundos enteros y no pasa de 30 (más que eso ya es una espera, no una cuenta).
// - Si el valor no es válido, se usa el valor por defecto y se devuelve un aviso.
function parseSongCountdown(value) {
  const defaultCountdown = { seconds: DEFAULT_SONG_COUNTDOWN_SECONDS, warning: null };
  if (value === undefined || value === null) return defaultCountdown;
  const raw = String(value).trim();
  if (raw === "") return defaultCountdown;

  const seconds = Number(raw);
  if (!Number.isFinite(seconds) || seconds < 0) {
    return {
      seconds: DEFAULT_SONG_COUNTDOWN_SECONDS,
      warning: `SONG_COUNTDOWN_SECONDS="${value}" no es válido (usa un número de segundos, o 0 para no mostrar la cuenta regresiva): se usan ${DEFAULT_SONG_COUNTDOWN_SECONDS} segundos.`,
    };
  }
  if (seconds > MAX_SONG_COUNTDOWN_SECONDS) {
    return {
      seconds: MAX_SONG_COUNTDOWN_SECONDS,
      warning: `SONG_COUNTDOWN_SECONDS="${value}" es demasiado: se usan ${MAX_SONG_COUNTDOWN_SECONDS} segundos, el máximo.`,
    };
  }
