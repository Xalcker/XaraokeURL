...
// Si una búsqueda no trae nada, se prueba con el siguiente sufijo: primero la versión karaoke, luego
// la instrumental y al final lo que se escribió, tal cual.
const SEARCH_FALLBACK_ORDER = ["karaoke", "instrumental", "none"];
...
// Interpreta SEARCH_RESULTS: cuántos resultados de YouTube se muestran. Devuelve
// { limit, warning }. Sin definir (o vacío) son 5; un entero fuera de 5–10 se
// ajusta al extremo más cercano; algo que no es un entero vuelve al valor por
// defecto. En los dos últimos casos se devuelve un aviso.
function parseSearchResultLimit(value) {
  if (value === undefined || value === null || String(value).trim() === "") {
    return { limit: MIN_SEARCH_RESULTS, warning: null };
  }
  const n = Number(String(value).trim());
  if (!Number.isInteger(n)) {
    return {
      limit: MIN_SEARCH_RESULTS,
      warning: `SEARCH_RESULTS="${value}" no es válido (usa un entero de ${MIN_SEARCH_RESULTS} a ${MAX_SEARCH_RESULTS}): se usan ${MIN_SEARCH_RESULTS}.`,
    };
  }
  const limit = Math.min(MAX_SEARCH_RESULTS, Math.max(MIN_SEARCH_RESULTS, n));
  if (limit !== n) {
    return {
      limit,
      warning: `SEARCH_RESULTS=${n} está fuera del rango permitido (${MIN_SEARCH_RESULTS} a ${MAX_SEARCH_RESULTS}): se usan ${limit}.`,
    };
  }
...
function normalizeChannelName(name) {
  return String(name || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "") // marcas de acento (por código: literales serían invisibles)
    .trim()
    .toLowerCase();
}

// Reordena los resultados de una búsqueda para que los de canales de los que ya
// se descargó algo salgan primero (primero el canal con más descargas). Dentro de
// cada grupo se conserva el orden que dio YouTube. `downloadedChannels` es la
// lista de canales de las descargas existentes, con repeticiones.
function rankByKnownChannels(results, downloadedChannels) {
  const downloadsPerChannel = new Map();
  for (const channel of downloadedChannels) {
    const key = normalizeChannelName(channel);
    if (key) downloadsPerChannel.set(key, (downloadsPerChannel.get(key) || 0) + 1);
  }
  if (downloadsPerChannel.size === 0) return results;
  const score = (result) => downloadsPerChannel.get(normalizeChannelName(result.channel)) || 0;
...
// Busca con cada sufijo de SEARCH_FALLBACK_ORDER hasta que alguno traiga resultados. Devuelve
// { results, suffix }, con el sufijo que los trajo (se guarda junto a la descarga). `search` se
// inyecta en las pruebas.
async function searchYoutubeWithFallback(query, { limit, search = searchYoutube } = {}) {
  let suffix = SEARCH_FALLBACK_ORDER[0];
  for (suffix of SEARCH_FALLBACK_ORDER) {
    const results = await search(query, { limit, suffix });
    if (results.length > 0) return { results, suffix };
  }
